...
# ETF Daily Returns 
ETFfxReturns <- read_excel("/Users/yihan/Downloads/BT2201Finance/ETFfxReturns.xlsx")
...
portfolioreturns <- dplyr::select(ETFfxReturns, -c(2,3))
...
## Mean Returns of Market, Risk-free Asset, ETFs
...
mean_ret <- data.frame(colMeans(numeric_returns, na.rm = T)) * 252
colnames(mean_ret) <- "Mean Annual Returns"
ETFmean_ret <- colMeans(ETFreturns, na.rm = T)
...
TRADING_DAYS <- 252
cov_mat <- cov(ETFreturns) * TRADING_DAYS
...
# We considered portfolios that matter the most to investors... 

- **Minimum Variance portfolio**
- **Market/Tangency portfolio**

Firstly, we assigned random weights to 16 ETFs and calculated the annualized portfolio returns and portfolio risk (sd). To find the optimal portfolio with the minimum variance and maximum Sharpe ratio, we looped this process 5000 times.  
...
```{r min-variance and tangency, echo=FALSE, warning=FALSE, fig.width = 10, fig.height = 5}
...
For in-sample testing, we benchmarked our portfolio against the following 60/40 stocks-bonds portfolio.\
...
## Portfolio vs 60/40 Benchmark Returns

We compared returns over the past ~20 years to observe how our portfolio performs 
...
#PLOTTING 
plotreturns <- plot(portwealthindex)
...
# Portfolio vs 60/40 Benchmark Statistics
...
rownames(finalstats) <- c("Max SR Portfolio", "Min Variance Portfolio","1/N Weighted Portfolio", "60/40 Benchmark")
